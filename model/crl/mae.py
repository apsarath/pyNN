...
class MAE(object):
    ...
    def init(self, numpy_rng, theano_rng=None, l_rate=0.01, optimization="sgd", tied=False, n_visible_left=None, n_visible_right=None, n_hidden=None, W_left=None, W_right=None, b=None, W_left_prime=None, W_right_prime=None, b_prime_left=None, b_prime_right=None, input_left=None, input_right=None, hidden_activation="sigmoid", output_activation="sigmoid", loss_fn = "squarrederror", op_folder=None):
    ...
    def train_left(self):
        ...
        if self.tied:
            curr_params = [self.W_left, self.b, self.b_prime_left, self.W_right, self.b_prime_right]
            curr_param_names = ["W_left", "b", "b_prime_left", "W_right", "b_prime_right"]
        else:
            curr_params = [self.W_left, self.b, self.b_prime_left, self.W_left_prime, self.W_right_prime, self.b_prime_right]
            curr_param_names = ["W_left", "b", "b_prime_left", "W_left_prime", "W_right_prime", "b_prime_right"]
    ...
    def train_right(self):
        ...
        if self.tied:
            curr_params = [self.W_right, self.b, self.b_prime_right, self.W_left, self.b_prime_left]
            curr_param_names = ["W_right", "b", "b_prime_right", "W_left", "b_prime_left"]
        else:
            curr_params = [self.W_right, self.b, self.b_prime_right, self.W_right_prime, self.W_left_prime, self.b_prime_left]
            curr_param_names = ["W_right", "b", "b_prime_right", "W_right_prime", "W_left_prime", "b_prime_left"]
    ...
    def load(self, folder, input_left=None, input_right=None):
        ...
        self.init(plist["numpy_rng"], theano_rng=plist["theano_rng"], l_rate=plist["l_rate"], optimization=plist["optimization"],
                  tied=plist["tied"], n_visible_left=plist["n_visible_left"], n_visible_right=plist["n_visible_right"], n_hidden=plist["n_hidden"],
                  W_left=folder+"W_left", W_right=folder+"W_right", b=folder+"b", W_left_prime=folder+"W_left_prime",
...
def trainMAE(src_folder, sct_folder, tgt_folder, batch_size = 20, training_epochs=40, use_valid=False, l_rate=0.01, optimization="sgd", tied=False, n_visible_left=None, n_visible_right=None, n_hidden=None,  W_left=None, W_right=None, b=None, W_left_prime=None, W_right_prime=None, b_prime_left=None, b_prime_right=None, hidden_activation="sigmoid", output_activation="sigmoid", loss_fn = "squarrederror"):
    ...
    common_cost, common_updates = model.train_common()
    ...
    mtrain_left = theano.function([index], left_cost,updates=left_updates,givens=[(x_left, train_set_x_left[index * batch_size:(index + 1) * batch_size]),(x_right, train_set_x_right[index * batch_size:(index + 1) * batch_size])])

    right_cost, right_updates = model.train_right()
    mtrain_right = theano.function([index], right_cost,updates=right_updates,givens=[(x_left, train_set_x_left[index * batch_size:(index + 1) * batch_size]),(x_right, train_set_x_right[index * batch_size:(index + 1) * batch_size])])
